    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-b from-white to-[#fae8e0]/40">
        <div className="max-w-7xl mx-auto px-6 py-12 md:py-16">
          <div className="max-w-3xl">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold text-[#3d3d3a] mt-2">Central de ajuda</h1>
            <div className="h-2 w-44 rounded-full bg-[#d97757] my-6" />
            <p className="text-[#6b6b68] text-base md:text-lg">Perguntas frequentes para você começar com confiança.</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 md:py-14">
        <div className="max-w-5xl mx-auto px-6 grid gap-4">
          {faqs.map((item, i) => (
            <div key={i} className="border border-[rgba(217,119,87,0.15)] rounded-2xl p-5 bg-white hover:border-[rgba(217,119,87,0.3)] transition-colors">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 w-8 h-8 rounded-md bg-[#fef5e7] text-[#d97757] grid place-items-center">
                  <HelpCircle className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-base font-semibold text-[#3d3d3a]">{item.q}</h3>
                  <p className="text-[#6b6b68] text-sm mt-1">{item.a}</p>
                </div>
              </div>
            </div>
          ))}
